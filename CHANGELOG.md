...
- **Tag 触发链路**：`changesets-release.yml` 从默认 `GITHUB_TOKEN` 切到 classic PAT（`secrets.RELEASE_PAT`，scopes：`repo` + `workflow`）。`GITHUB_TOKEN` 因 GitHub 防递归设计无法触发其他 workflow——即便能推 tag，`release.yml` 也不会被触发。期间还试过 fine-grained PAT，实测同样不触发；只有 classic PAT 能让推 tag 真正 fan-out 到 `release.yml`。详见 `docs/deployment/release.md` §六「为什么是 classic PAT」。
