...
| 4.5 交叉验证         | Phase 4 Actor (`packages/workflow/src/actors/cross-validate.ts`) | ✅ 已实现 |
| 4.6 定量测算         | Phase 5 报告综合中内含定量分析逻辑                              | ✅ 已实现 |
| 4.7 综合判断与评分   | Phase 5 Actor (`packages/workflow/src/actors/synthesize-report.ts`) + 评分规则 | ✅ 已实现 |
| 4.8 行动建议         | Phase 5 报告第七节（如果推进/暂缓/否定）                        | ✅ 已实现 |
| 4.9 引用管理         | Phase 5 报告模板中 [来源编号] 引用 + 第八节参考来源表           | ✅ 已实现 |
...
| 4.13 评分机制        | Phase 5 加权评分 + one-veto 规则 + 区间分                       | ✅ 已实现 |
| 4.14 长度自适应      | Phase 2 确定 complexity → Phase 5 按 REPORT_CHAR_TARGETS 生成   | ✅ 已实现 |
| 4.15 报告迭代        | `POST /api/research/:id/iterate` API                            | ✅ 已实现 |
