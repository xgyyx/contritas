...
- Phase 6 加固与生产就绪 🚧（11 个子领域，**R2 审视 2026-05-22 新增 19 项工单 + 1 项重开**）：
  ...
  - 6.4 SSE 可靠性（核心 7 项 ✅；**R2 6.4.8 ✅** 客户端 lastEventId 断点续传；**R2** 6.4.9 关闭忙等 待办）
  ...
  - 6.7 测试覆盖（API 集成 + e2e + 4 个 workflow actor 单测 + BullMQ clarification 回归 + apps/web 4 组件 RTL，共 183 测试 ✅；**R2** 6.7.7 覆盖盲点 / 6.7.8 invokeActor helper 抽离 待办）
  ...
  - **6.11 前端代码质量与性能（R2 新建）**：6.11.1 ResearchStore 无界数组 / 6.11.2 MarkdownRenderer 重渲染 / 6.11.3 setReport as any / 6.11.4 effect deps 噪音 待办
...
- Oracle Cloud Always Free 部署：`docs/deployment/oracle-free-tier.md`（部署产物：`deploy/oracle-free-tier/`）
